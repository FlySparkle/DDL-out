...
import 'presentation/widgets/board_navigation.dart';
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final fixedNavigation =
            settings.sidebarMode == SidebarMode.fixed &&
            BoardNavigationLayout.canUseFixed(constraints.maxWidth);
        return Scaffold(
          drawer: fixedNavigation ? null : const BoardNavigationDrawer(),
          drawerEnableOpenDragGesture: !fixedNavigation,
          appBar: AppBar(
            automaticallyImplyLeading: !fixedNavigation,
          ...
          body: board.when(
            data: (snapshot) {
              final content = BoardContent(snapshot: snapshot);
              if (!fixedNavigation) return content;
              return FixedBoardNavigation(child: content);
            },
