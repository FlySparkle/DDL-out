...
| Windows ARM64 | Portable ZIP | 可构建 | `Release` |
| Windows | MSIX | 预留 | 配置应用身份与签名后接入扩展工作流 |
| Linux x64 | Bundle、DEB、RPM | 可构建 | `Release` |
| Android arm64-v8a | Release APK | 可构建 | `CI` 与 `Release` |
| iOS | 已签名 IPA | 预留 | `Apple Platform Builds` 手动工作流 |
| macOS | 未签名 DMG | 预留 | `Apple Platform Builds` 手动工作流 |
